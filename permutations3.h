...
void CalculatePermu( vector<int> &permu, vector<int> midNum, vector<int> fisPermu){   //get permutation from medium number ( descending radix method)
	int size0 = permu.size();
	vector<int> permuIndex( size0,0);
	int bankLoc = midNum[size0-2];
	permu[size0-1-bankLoc] = fisPermu[ size0-1 ];
	permuIndex[size0-1-bankLoc] = 1;
	for(int i =1; i<size0-1; i++){
		bankLoc = midNum[size0-2-i];
